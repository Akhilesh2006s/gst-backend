...
const Vendors: React.FC = () => {
  ...
  const fetchVendors = async () => {
    ...
    try {
      ...
      const response = await fetch('https://gst-backend-production-8aff.up.railway.app/api/vendors', {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = editingVendor 
        ? `https://gst-backend-production-8aff.up.railway.app/api/vendors/${editingVendor._id}`
        : 'https://gst-backend-production-8aff.up.railway.app/api/vendors';
  ...
  const handleDelete = async (vendorId: string) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`https://gst-backend-production-8aff.up.railway.app/api/vendors/${vendorId}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleDownload = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch('https://gst-backend-production-8aff.up.railway.app/api/vendors/download', {
        headers: { Authorization: `Bearer ${token}` }
      });

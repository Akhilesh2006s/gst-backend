...
const Customers: React.FC = () => {
  ...
  const fetchCustomers = async () => {
    try {
      ...
      const response = await fetch(`https://gst-backend-production-8aff.up.railway.app/api/customers?${params}`, {
        headers: {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = editingCustomer 
        ? `https://gst-backend-production-8aff.up.railway.app/api/customers/${editingCustomer._id}`
        : 'https://gst-backend-production-8aff.up.railway.app/api/customers';
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`https://gst-backend-production-8aff.up.railway.app/api/customers/${id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  const handleDownload = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch('https://gst-backend-production-8aff.up.railway.app/api/customers/download', {

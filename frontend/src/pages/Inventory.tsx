...
const Inventory: React.FC = () => {
  ...
  const fetchInventory = async () => {
    ...
    try {
      ...
      const response = await fetch(`https://gst-backend-production-8aff.up.railway.app/api/inventory?${params}`, {
  ...
  const fetchStats = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch('https://gst-backend-production-8aff.up.railway.app/api/inventory/stats', {
        headers: { Authorization: `Bearer ${token}` }
      });

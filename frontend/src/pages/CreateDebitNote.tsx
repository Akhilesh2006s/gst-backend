...
const CreateDebitNote: React.FC = () => {
  ...
  const fetchPurchases = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch('https://gst-backend-production-8aff.up.railway.app/api/purchases', {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch('https://gst-backend-production-8aff.up.railway.app/api/debit-notes', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,

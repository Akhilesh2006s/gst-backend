...
const Reports: React.FC = () => {
  ...
  const fetchBusinessSummary = async () => {
    ...
    try {
      ...
      const response = await fetch(`https://gst-backend-production-8aff.up.railway.app/api/reports/business-summary?${params}`, {
  ...
  const downloadExcel = async () => {
    try {
      ...
      const response = await fetch(`https://gst-backend-production-8aff.up.railway.app/api/reports/business-summary/excel?${params}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const downloadReport = async (reportType: string, format: 'excel' | 'json') => {
    try {
      ...
      const response = await fetch(`https://gst-backend-production-8aff.up.railway.app/api/reports/${reportType}?${params}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const downloadGSTR = async (type: 'gstr-1' | 'gstr-2b' | 'gstr-3b') => {
    try {
      ...
      const response = await fetch(`https://gst-backend-production-8aff.up.railway.app/api/reports/${type}?${params}`, {
        headers: { Authorization: `Bearer ${token}` }
      });

...
const StockOperationModal: React.FC<StockOperationModalProps> = ({ 
  ...
  const fetchItems = async () => {
    try {
      ...
      const response = await fetch('https://gst-backend-production-8aff.up.railway.app/api/items', {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`https://gst-backend-production-8aff.up.railway.app/api/inventory/${endpoint}`, {
        method: 'POST',
        headers: {

...
const BankAccountForm: React.FC<BankAccountFormProps> = ({ onSave }) => {
  ...
  const fetchBankAccounts = async () => {
    try {
      ...
      const response = await fetch('https://gst-backend-production-8aff.up.railway.app/api/bank-accounts', {
        headers: {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const url = editingId 
        ? `https://gst-backend-production-8aff.up.railway.app/api/bank-accounts/${editingId}`
        : 'https://gst-backend-production-8aff.up.railway.app/api/bank-accounts';
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`https://gst-backend-production-8aff.up.railway.app/api/bank-accounts/${id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  const handleSetDefault = async (id: string) => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`https://gst-backend-production-8aff.up.railway.app/api/bank-accounts/${id}/set-default`, {
        method: 'PATCH',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'

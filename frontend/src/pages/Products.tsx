...
const Products: React.FC = () => {
  ...
  const fetchProducts = async () => {
    ...
    try {
      ...
      const response = await fetch(`https://gst-backend-production-8aff.up.railway.app/api/items?${params}`, {
  ...
  const fetchStats = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch('https://gst-backend-production-8aff.up.railway.app/api/items/stats', {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleDelete = async (productId: string) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`https://gst-backend-production-8aff.up.railway.app/api/items/${productId}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleBulkUpload = async (file: File) => {
    try {
      ...
      const response = await fetch('https://gst-backend-production-8aff.up.railway.app/api/items/bulk-upload', {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}` },
        body: formData
      });

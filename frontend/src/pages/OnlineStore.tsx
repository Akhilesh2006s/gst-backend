...
const OnlineStore: React.FC = () => {
  ...
  const fetchStore = async () => {
    ...
    try {
      ...
      const response = await fetch('https://gst-backend-production-8aff.up.railway.app/api/stores', {
  ...
  const publishStore = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`https://gst-backend-production-8aff.up.railway.app/api/stores/${store._id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`

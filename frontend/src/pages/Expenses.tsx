...
const Expenses: React.FC = () => {
  ...
  const fetchExpenses = async () => {
    ...
    try {
      ...
      const response = await fetch(`https://gst-backend-production-8aff.up.railway.app/api/expenses?${params}`, {
  ...
  const fetchStats = async () => {
    try {
      ...
      const response = await fetch(`https://gst-backend-production-8aff.up.railway.app/api/expenses/stats?${params}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleDelete = async (expenseId: string) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`https://gst-backend-production-8aff.up.railway.app/api/expenses/${expenseId}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` }
      });

...
const CreateProforma: React.FC = () => {
  ...
  const fetchCompanies = async () => {
    try {
      ...
      const response = await fetch('https://gst-backend-production-8aff.up.railway.app/api/company', {
  ...
  const calculateTotals = (items: ProformaItem[]) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch('https://gst-backend-production-8aff.up.railway.app/api/proformas', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        console.error('Proforma creation failed:', errorData);
        console.error('Response status:', response.status);
        setError(errorData.message || `Failed to create proforma (Status: ${response.status})`);
      }
    } catch (error) {
    ...
    <DashboardLayout>
      <div className="p-6">
        <div className="mb-6">
          ...
          <h1 className="text-3xl font-bold text-gray-900">Create Proforma Invoice</h1>
          <p className="mt-2 text-gray-600">Create a professional proforma invoice for your customer</p>
        </div>
            ...
            <CardContent>
              {formData.items.length === 0 ? (
                <div className="text-center py-8 text-gray-500">
                  ...
                  <p>No items added yet. Click "Add Item" to start building your proforma.</p>
                </div>
              ) : (
                <div className="space-y-4">

...
const CustomerStatements: React.FC = () => {
  ...
  const fetchCustomerStatement = async () => {
    ...
    try {
      ...
      const response = await fetch(`https://gst-backend-production-8aff.up.railway.app/api/payments/customer/${selectedCustomer}?${params}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleDownloadStatement = async () => {
    ...
    try {
      ...
      const response = await fetch(`https://gst-backend-production-8aff.up.railway.app/api/payments/customer/${selectedCustomer}/download?${params}`, {
        headers: { Authorization: `Bearer ${token}` }
      });

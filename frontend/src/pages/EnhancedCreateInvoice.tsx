...
const EnhancedCreateInvoice: React.FC = () => {
  ...
  const fetchSignature = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch('https://gst-backend-production-8aff.up.railway.app/api/user/signature', {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (response.ok) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch('https://gst-backend-production-8aff.up.railway.app/api/invoices', {
        method: 'POST',
        headers: {
